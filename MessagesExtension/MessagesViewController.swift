...
//  MessagesExtension
//
//  Created by Elias Crouch on 10/14/16.
//  Copyright © 2016 Elias Crouch. All rights reserved.
//
...
class MessagesViewController: MSMessagesAppViewController, MSStickerBrowserViewDataSource {
    ...
    func loadStickers() {
        for s in stickerList {
            if let url = Bundle.main.url(forResource: s[0], withExtension: "png") {
                do {
                    let sticker = try MSSticker(contentsOfFileURL: url, localizedDescription: s[1])
